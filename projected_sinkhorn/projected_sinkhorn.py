...
def lamw(x):
    ...
    # print("lambertw function evaluation time {:.3f}ms".format(start.elapsed_time(end)))
...
def log_sinkhorn(X, Y, C, epsilon, lam, verbose=False, plan=False,
   objective='2norm', maxiters=50, return_objective=False, return_iters=False, termination=None):
    """ 
    if objective == '2norm': 
        ...
        with torch.no_grad(): 
            while True: 
                ...
                if any_nan(exp_alpha) or any_inf(exp_alpha) or \
                   any_nan(exp_beta) or any_nan(exp_beta):
                    print('Overflow error: in logP_sinkhorn')
                    return None, None

        if return_iters:
            return beta/lam + Y, i - 1
        else:
            return beta/lam + Y
